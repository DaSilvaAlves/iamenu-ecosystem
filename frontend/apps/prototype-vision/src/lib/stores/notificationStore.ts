...
const useNotificationStore = create<NotificationState>((set, get) => ({
   ...
   */
  fetchNotifications: async (reset: boolean = false) => {
    ...
    try {
      ...
      const data = response.data as NotificationApiResponse;
      const newNotifications: Notification[] = data.notifications || data.data || [];
   ...
   */
  setWsConnected: (connected: boolean) => set({ wsConnected: connected, wsError: null }),

  /**
   * Set WebSocket error
   */
  setWsError: (error: string | null) => set({ wsError: error, wsConnected: false }),
