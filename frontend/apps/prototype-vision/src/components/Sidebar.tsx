...
const NavItem: React.FC<NavItemProps> = ({ icon: Icon, label, to, badge, isCollapsed = false }) => {
...
const SidebarSection: React.FC<SidebarSectionProps> = ({ title, children, hasChevron = true, isCollapsed = false }) => {
    if (isCollapsed) {
        return (
...
interface SidebarContentProps {
    ...
    isComunidadeActive: boolean;
    setSelectedGroup: (group: Group) => void;
    getGroupIcon: (category: string) => LucideIcon;
    isCollapsed?: boolean;
}
            ...
            {!isCollapsed && (
                <SidebarSection title="Grupos da Comunidade" isCollapsed={isCollapsed}>
                    ...
                    <div style={{ padding: '8px 12px', marginTop: '8px' }}>
                        ...
                        >
                            Ver todos os {groups.length} grupos &rarr;
                        </Link>
                    </div>
                </SidebarSection>
            )}
...
const Sidebar: React.FC<SidebarProps> = ({ setSelectedGroup }) => {
    ...
    const { isMobile, isCollapsed, isOpen, close } = useSidebarStore();
    ...
    const loadGroups = async (): Promise<void> => {
        try {
            const data = await CommunityAPI.getGroups({ limit: 10 }) as Record<string, unknown>;
            setGroups((data.data as GroupData[]) || []);
        } catch (err: unknown) {
    ...
    const getGroupIcon = (category: string): LucideIcon => {
