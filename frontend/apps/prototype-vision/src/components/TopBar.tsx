...
import { Search, Bell, MessageSquare, Bookmark, User, Menu } from 'lucide-react';
...
const TopBar: React.FC = () => {
    ...
    const loadNotifications = async (): Promise<void> => {
        try {
            const data = await CommunityAPI.getNotifications({ limit: 10 }) as Record<string, unknown>;
            setNotifications((data.data as NotificationData[]) || []);
    ...
    const loadUserProfile = async (): Promise<void> => {
        try {
            ...
            if (userId) {
                const profileData = await CommunityAPI.getProfile(userId) as Record<string, unknown>;
                setUserProfile(profileData.data as UserProfileData);
            }
        } catch (err: unknown) {
