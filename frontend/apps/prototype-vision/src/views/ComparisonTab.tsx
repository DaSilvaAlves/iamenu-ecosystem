...
interface SelectedOffer extends Offer {
    productId: string;
    productName: string;
}
...
const StatsCard = ({ icon, label, value, trend, trend_label }: StatsCardProps) => (
...
const SupplierCard = ({ offer, product, isBest, isSelected, onToggleOfferSelection }: SupplierCardProps) => {
    ...
    const renderHistoryChart = (priceHistory?: PriceHistoryEntry[]) => {
        ...
        return (
            <div className="h-8 w-full flex items-end justify-between gap-1 opacity-80">
                {priceHistory.map((entry, index) => {
                    const normalizedHeight = ((entry.price - minPrice) / priceRange) * 90 + 10;
                    const barColor = index === priceHistory.length - 1 ? 'bg-brand-orange' : 'bg-brand-orange/20';
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-1 mt-1">
                            <div className="h-1.5 w-16 bg-[#333] rounded-full overflow-hidden">
                                {offer.Supplier?.ratingAvg !== undefined ? (
            ...
            <div className="p-6 flex-1 flex flex-col gap-5 bg-surface-card">
                <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                 ...
                 <div className="mt-2 pt-4 border-t border-border">
                    ...
                    {renderHistoryChart(offer.priceHistory)}
                </div>
...
const ComparisonTab = () => {
    ...
    const [stats, setStats] = useState<Stats>({ avgPrice: 0, bestPrice: 0, potentialSaving: 0 });
    ...
    const handleSearch = async (advancedFilters: AdvancedFilters = {}) => {
        ...
        try {
            ...
            if (mainProduct && mainProduct.offers.length > 0) {
                const validOffers = mainProduct.offers.filter(o => o.price && o.price > 0);
                ...
                const allUniqueFilters = [...uniqueCategories, ...uniqueSubcategories] as string[];
                setAvailableFilters(Array.from(new Set(allUniqueFilters)));
            ...
            const message = e instanceof Error ? e.message : 'Falha ao buscar produtos. Verifique a consola.';
            setError(message);
            setProducts([]);
        } finally {
            setLoading(false);
    ...
    const bestOffer = mainProduct ? mainProduct.offers.reduce((best, current) => (current.price < best.price ? current : best), mainProduct.offers[0]) : null;

    const handleToggleOfferSelection = (offerToToggle: Offer, productForOffer: Product) => {
    ...
    const handleApplyAdvancedFilters = () => {
        const filtersToApply: AdvancedFilters = {
    ...
    return (
        <div className="max-w-7xl mx-auto flex flex-col gap-8 pb-20">
            ...
            <div className="border-b border-border">
                <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar -mb-px">
                    <button
            ...
            {!loading && !error && !mainProduct && activeTab === 'pesquisa' && (
            ...
            {!loading && !error && mainProduct && activeTab === 'pesquisa' && (
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                        {mainProduct.offers.sort((a, b) => a.price - b.price).map(offer => (
                            <SupplierCard
            ...
            {selectedOffers.length > 0 && (
                <div className="fixed bottom-6 left-1/2 md:left-[calc(50%+8rem)] -translate-x-1/2 bg-[#1E1E1E] border border-brand-orange text-white px-6 py-3 rounded-xl shadow-2xl z-40 flex items-center gap-4">
                    ...
                    <button
                        onClick={() => console.log('Comparar Detalhado para:', selectedOffers)}
