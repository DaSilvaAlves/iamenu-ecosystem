...
interface BestProposal extends Quote {
...
const ResponsesTab = ({ rfqId: propRfqId }: ResponsesTabProps) => { // Accept rfqId as a prop
    ...
    const [responses, setResponses] = useState<Quote[] | null>(null);
    ...
    const calculateTotalPrice = (items: QuoteItem[]) => items.reduce((acc, item) => acc + (item.total || 0), 0);
    ...
    let bestProposal: BestProposal | null = null;
    let minTotalPrice = Infinity;
    let fastestDelivery: BestProposal | null = null;
    ...
    const bestPriceValue = bestProposal ? (bestProposal as BestProposal).calculatedTotalPrice : 0;
    const bestDeliverySupplier = fastestDelivery ? (fastestDelivery as BestProposal).supplier?.companyName : 'N/A';
    const bestDeliveryTerm = fastestDelivery ? (fastestDelivery as BestProposal).deliveryTerms : 'N/A';
    ...
    const handleAcceptProposal = async (quoteId: string) => {
        ...
        try {
            ...
            if (!token) {
                toast.error('Autenticacao necessaria para aceitar propostas.');
            ...
            toast.dismiss();
            toast.error(`Falha ao aceitar proposta: ${err instanceof Error ? err.message : 'Unknown error'}`);
    ...
    const handleAcceptIndividualQuote = async (quoteId: string) => {
        if (!quoteId) {
            toast.error('ID da proposta invalido.');
            return;
        }
        ...
        try {
            ...
            toast.dismiss();
            toast.error(`Falha ao aceitar proposta individual: ${err instanceof Error ? err.message : 'Unknown error'}`);
    ...
    if (error) {
        return (
            ...
            >
                ...
                <p className="text-white/10 font-bold mt-2 uppercase text-xs tracking-[0.4em]">Nao foi possivel carregar as respostas</p>
            </motion.div>
        );
    }
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                >
                    <div>
                        <p className="text-sm text-text-secondary">Melhor Proposta (Preco)</p>
                ...
                >
                    <div>
                        <p className="text-sm text-text-secondary">Entrega Mais Rapida</p>
            ...
            {bestProposal && (
                ...
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-text-secondary">
                        <div>
                            <p>Fornecedor: <span className="text-white font-semibold">{(bestProposal as BestProposal).supplier?.companyName || 'Desconhecido'}</span></p>
                            <p>Preco Total: <span className="text-green-400 font-semibold">
                                {(bestProposal as BestProposal).calculatedTotalPrice.toLocaleString('pt-PT', { style: 'currency', currency: 'EUR' })}</span></p>
                        </div>
                        <div>
                            <p>Condicoes de Entrega: <span className="text-white font-semibold">{(bestProposal as BestProposal).deliveryTerms}</span></p>
                            <p>Condicoes de Pagamento: <span className="text-white font-semibold">{(bestProposal as BestProposal).paymentTerms}</span></p>
                        </div>
                    </div>
                    ...
                    <ul className="list-disc list-inside text-text-muted">
                        {(bestProposal as BestProposal).items.map((item, index) => (
                    ...
                    <div className="flex justify-end mt-6 gap-3">
                        ...
                        >Ver Detalhes</button>
                        <button
                            onClick={() => handleAcceptProposal((bestProposal as BestProposal).id)}
            ...
            {/* Header com filtros e botoes de accao */}
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-3 relative">
                    ...
                    >
                        Filtrar por Preco {sortBy === 'priceAsc' ? '\u2191' : '\u2193'}
                    </button>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="text-text-secondary text-xs uppercase font-bold tracking-wider">
                            ...
                            <th className="px-6 py-3 border-b border-border-dark text-center">Preco Total</th>
                            <th className="px-6 py-3 border-b border-border-dark text-center">Condicoes</th>
                            ...
                            <th className="px-6 py-3 border-b border-border-dark text-right">Accoes</th>
                        </tr>
                    </thead>
            ...
            <footer className="mt-8 pt-6 border-t border-border-dark flex justify-between items-center text-xs text-text-secondary">
                <p>&copy; 2024 iaMenu Ecosystem. Todos os direitos reservados.</p>
