...
const DashboardBI = ({ setView }: DashboardBIProps) => {
  const [activeTab, setActiveTab] = useState<string>('overview');
  const [selectedPeriod, setSelectedPeriod] = useState<string>('semana');
  const [trendPeriod, setTrendPeriod] = useState<string>('hoje');
  const [stats, setStats] = useState<DashboardStats | null>(null);
  const [salesTrends, setSalesTrends] = useState<SalesTrend[]>([]);
  const [aiPrediction, setAIPrediction] = useState<unknown>(null);
  const [topProducts, setTopProducts] = useState<Product[]>([]);
  const [alerts, setAlerts] = useState<Alert[]>([]);
  const [menuEngineering, setMenuEngineering] = useState<MenuEngineeringData | null>(null);
  const [demandForecast, setDemandForecast] = useState<DemandForecastData | null>(null);
  const [peakHoursHeatmap, setPeakHoursHeatmap] = useState<PeakHoursData | null>(null);
  ...
  const loadDashboardData = async (): Promise<void> => {
    try {
      ...
      if (activeTab === 'menu') {
        ...
        const menuRes = await DashboardAPI.getMenuEngineering() as { data: MenuEngineeringData };
        setMenuEngineering(menuRes.data);
      } else if (activeTab === 'forecast') {
        // Carregar apenas AI Forecast
        const [forecastRes, heatmapRes] = await Promise.all([
          DashboardAPI.getDemandForecast() as Promise<{ data: DemandForecastData }>,
          DashboardAPI.getPeakHoursHeatmap() as Promise<{ data: PeakHoursData }>
        ]);
        ...
        const benchmarkRes = await DashboardAPI.getBenchmark() as { data: BenchmarkData };
        ...
        const [statsRes, trendsRes, predictionRes, productsRes, alertsRes] = await Promise.all([
          DashboardAPI.getStats(selectedPeriod) as Promise<{ data: DashboardStats }>,
          DashboardAPI.getSalesTrends(trendPeriod) as Promise<{ data: { data: SalesTrend[] } }>,
          DashboardAPI.getAIPrediction() as Promise<{ data: unknown }>,
          DashboardAPI.getTopProducts(5) as Promise<{ data: Product[] }>,
          DashboardAPI.getAlerts() as Promise<{ data: Alert[] }>
        ]);
  ...
  const handleNavigateToFoodCost = (): void => {
    if (setView) {
      setView('foodcost');
    } else {
  ...
  const showComingSoon = (featureName: string): void => {
  ...
  const handleAlertAction = (alert: Alert): void => {
    ...
    return [...products].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case 'sales':
          ...
          break;
        case 'classification':
          const order: Record<string, number> = { Star: 1, Gem: 2, Popular: 3, Dog: 4 };
          aValue = order[a.category as string] || 5;
          bValue = order[b.category as string] || 5;
          break;
        default:
          return 0;
      }
  ...
  const handleExportPDF = (): void => {
    ...
    if (stats) {
      ...
      const statsData: string[][] = [
        ...
        ['Receita Bruta', stats.receita.formatted || '€0', stats.receita.trend],
        ['Clientes (Covers)', (stats.clientes.value || 0).toString(), stats.clientes.trend],
        ['Ticket Médio', stats.ticketMedio.formatted || '€0', stats.ticketMedio.trend],
        ['Food Cost %', stats.foodCost.formatted || '0%', stats.foodCost.trend]
      ];
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        ...
        105,
        (doc.internal.pageSize.height as number) - 10,
        { align: 'center' }
      );
      doc.text(
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/5 border border-yellow-500/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-3xl font-black text-yellow-400">{(menuEngineering.stars as unknown[])?.length || 0}</span>
              </div>
            ...
            <div className="bg-gradient-to-br from-green-500/10 to-green-600/5 border border-green-500/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-4xl">💎</span>
                <span className="text-3xl font-black text-green-400">{(menuEngineering.gems as unknown[])?.length || 0}</span>
              </div>
            ...
            <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-500/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-4xl">🐴</span>
                <span className="text-3xl font-black text-blue-400">{(menuEngineering.populars as unknown[])?.length || 0}</span>
              </div>
            ...
            <div className="bg-gradient-to-br from-red-500/10 to-red-600/5 border border-red-500/20 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-4xl">🐕</span>
                <span className="text-3xl font-black text-red-400">{(menuEngineering.dogs as unknown[])?.length || 0}</span>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {((menuEngineering.opportunities as any)?.gems?.count || 0) > 0 && (
              <div className="bg-gradient-to-br from-green-500/10 to-transparent border border-green-500/20 rounded-xl p-4">
                ...
                <p className="text-white/80 text-sm mb-3">{(menuEngineering.opportunities as any)?.gems?.suggestion || ''}</p>
                <p className="text-green-400 font-bold text-xs">Potencial: +€{((menuEngineering.opportunities as any)?.gems?.potential || 0).toFixed(0)}</p>
              </div>
            )}
            {((menuEngineering.opportunities as any)?.populars?.count || 0) > 0 && (
              <div className="bg-gradient-to-br from-blue-500/10 to-transparent border border-blue-500/20 rounded-xl p-4">
                ...
                <p className="text-white/80 text-sm mb-3">{(menuEngineering.opportunities as any)?.populars?.suggestion || ''}</p>
                <p className="text-blue-400 font-bold text-xs">Potencial: +€{((menuEngineering.opportunities as any)?.populars?.potential || 0).toFixed(0)}</p>
              </div>
            )}
            {((menuEngineering.opportunities as any)?.dogs?.count || 0) > 0 && (
              <div className="bg-gradient-to-br from-red-500/10 to-transparent border border-red-500/20 rounded-xl p-4">
                ...
                <p className="text-white/80 text-sm">{(menuEngineering.opportunities as any)?.dogs?.suggestion || ''}</p>
              </div>
            )}
          </div>
                ...
                <tbody>
                  {(() => {
                    const products: Product[] = [
                      ...((menuEngineering.stars || []) as unknown[]).map((p: unknown) => ({ ...(p as Record<string, unknown>), category: 'Star' })),
                      ...((menuEngineering.gems || []) as unknown[]).map((p: unknown) => ({ ...(p as Record<string, unknown>), category: 'Gem' })),
                      ...((menuEngineering.populars || []) as unknown[]).map((p: unknown) => ({ ...(p as Record<string, unknown>), category: 'Popular' })),
                      ...((menuEngineering.dogs || []) as unknown[]).map((p: unknown) => ({ ...(p as Record<string, unknown>), category: 'Dog' }))
                    ];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Stars */}
            {((menuEngineering.stars as unknown[]) || []).length > 0 && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <h3 className="text-white font-bold mb-4">⭐ Stars - Manter & Promover</h3>
                <div className="space-y-2">
                  {((menuEngineering.stars as unknown[]) || []).map((p: unknown) => {
                    const product = p as Record<string, unknown>;
                    return (
                      <div key={product.id as string} className="flex justify-between items-center p-3 bg-yellow-500/10 rounded-lg">
                        <div>
                          <p className="text-white font-semibold text-sm">{product.name as string}</p>
                          <p className="text-white/60 text-xs">{product.sales as number} vendas • Margem: {product.margin as string}%</p>
                        </div>
                        <p className="text-yellow-400 font-bold">€{((product.revenue as number) || 0).toFixed(0)}</p>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Gems */}
            {((menuEngineering.gems as unknown[]) || []).length > 0 && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <h3 className="text-white font-bold mb-4">💎 Gems - Promover Mais</h3>
                <div className="space-y-2">
                  {((menuEngineering.gems as unknown[]) || []).map((p: unknown) => {
                    const product = p as Record<string, unknown>;
                    return (
                      <div key={product.id as string} className="flex justify-between items-center p-3 bg-green-500/10 rounded-lg">
                        <div>
                          <p className="text-white font-semibold text-sm">{product.name as string}</p>
                          <p className="text-white/60 text-xs">{product.sales as number} vendas • Margem: {product.margin as string}%</p>
                        </div>
                        <p className="text-green-400 font-bold">€{((product.revenue as number) || 0).toFixed(0)}</p>
                      </div>
                    );
                  })}
                </div>
