...
import { CommunityAPI, Auth, getImageUrl } from '../services/api';
...
interface CommunityViewProps {
    ...
    setSelectedGroup?: (group: GroupSummary | null) => void;
}
...
const getUpcomingEvents = () => {
    const now = new Date();
    const events = [
        { daysFromNow: 5, title: 'Webinar: IA na Gestao de Stocks', time: '15:00 WET' },
...
const getTrendingTags = () => {
    return ['#DigitalGastro', '#Sustentabilidade', '#FoodCost', '#Inovacao', '#StaffRetention'];
};

const CommunityView = ({ selectedGroup, setSelectedGroup }: CommunityViewProps) => {
    ...
    const loadUserGroups = async () => {
        try {
            ...
            const data = await CommunityAPI.getUserGroups(userId) as { data?: GroupSummary[] };
    ...
    const loadPosts = async (groupId?: string | null) => {
        try {
            setLoading(true);
            ...
            const data = await CommunityAPI.getPosts(params) as { data?: Post[] };
            setPosts(data.data || []);
            setError(null);
        } catch (err: unknown) {
    ...
    const handleCreatePost = async (postData: PostFormData) => {
        try {
            await CommunityAPI.createPost(postData as unknown as PostFormData);
            setShowNewPostModal(false);
            loadPosts(); // Reload posts
        } catch (err: unknown) {
            alert('Erro ao criar post: ' + (err instanceof Error ? err.message : 'Unknown error'));
        }
    };
    ...
    const toggleComments = async (postId: string) => {
        ...
        if (!isExpanded) {
            ...
            if (!comments[postId]) {
                try {
                    const data = await CommunityAPI.getComments(postId) as { data?: Comment[] };
    ...
    const handleCreateComment = async (postId: string) => {
        ...
        if (!content || !content.trim()) {
            alert('Comentario nao pode estar vazio');
            return;
        }
        ...
        try {
            ...
            alert('Erro ao criar comentario: ' + (err instanceof Error ? err.message : 'Unknown error'));
        }
    };

    const handleReaction = async (postId: string, reactionType: string) => {
        try {
            const result = await CommunityAPI.toggleReaction(postId, reactionType) as { data: { action: string } };
            ...
            alert('Erro ao reagir: ' + (err instanceof Error ? err.message : 'Unknown error'));
        }
    };
    ...
    const formatTimeAgo = (dateString: string) => {
        ...
        return `ha ${Math.floor(seconds / 86400)} dias`;
    };
        ...
        >
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <p style={{ fontSize: '1.25rem', color: 'rgba(255,255,255,0.8)', maxWidth: '600px' }}>
                        Juntos estamos a revolucionar a restauracao em Portugal. Bem-vindos ao futuro!
                    </p>
                </div>
            </div>
            ...
            {feedTab === 'mygroups' && userGroups.length === 0 && !loading && (
                <div className="glass-panel" style={{
                    ...
                    <h3 style={{ marginBottom: '12px' }}>Ainda nao es membro de nenhum grupo</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '24px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    {!loading && posts.length === 0 && (
                        <div className="glass-panel" style={{ padding: '40px', textAlign: 'center' }}>
                            <p style={{ color: 'var(--text-muted)' }}>Ainda nao ha posts. Seja o primeiro a partilhar!</p>
                        </div>
                    )}
                    ...
                    {!loading && posts.map((post, index) => (
                        ...
                        >
                            ...
                            <div style={{
                                ...
                                >
                                    <MessageSquare size={16} /> {comments[post.id]?.length || post._count?.comments || 0} Comentarios
                                </div>
                                <div
                                    ...
                                    style={{
                                        ...
                                        cursor: 'pointer',
                                        transition: 'all 0.2s',
                                        color: (post.reactions?.like && post.reactions.like > 0) ? 'var(--primary)' : 'var(--text-muted)'
                                    }}
                                    onMouseEnter={(e) => {
                                    ...
                                    onMouseLeave={(e) => {
                                        ...
                                        e.currentTarget.style.color = (post.reactions?.like && post.reactions.like > 0) ? 'var(--primary)' : 'var(--text-muted)';
                                    }}
                                >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    {/* Eventos */}
                    <div className="glass-panel" style={{ padding: '24px' }}>
                        <h3 style={{ fontSize: '1rem', fontWeight: 'bold', marginBottom: '16px' }}>Eventos Proximos</h3>
                    ...
                    {/* Tendencias */}
                    <div className="glass-panel" style={{ padding: '24px' }}>
                        <h3 style={{ fontSize: '1rem', fontWeight: 'bold', marginBottom: '16px' }}>Tendencias</h3>
...
const NewPostModal = ({ onClose, onSubmit }: NewPostModalProps) => {
    ...
    useEffect(() => {
        ...
        CommunityAPI.getGroups({ limit: 20 }).then((data: unknown) => {
            const result = data as { data?: GroupSummary[] };
        ...
        <div style={{
            ...
            <div className="glass-panel" style={{ padding: '32px', maxWidth: '600px', width: '90%', maxHeight: '90vh', overflowY: 'auto', margin: 'auto' }} onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit}>
                    <div style={{ marginBottom: '16px' }}>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem' }}>Titulo *</label>
                        <input
                            type="text"
                            value={title}
                    ...
                    <div style={{ marginBottom: '16px' }}>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem' }}>Conteudo *</label>
                        <MentionInput
                            value={body}
                            onChange={setBody}
                            placeholder="Partilha a tua experiencia... (usa @username para mencionar)"
                            minHeight="150px"
                        />
                    </div>

                    <div style={{ marginBottom: '16px' }}>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem' }}>Tags (separadas por virgula)</label>
                        <input
                            type="text"
                            value={tags}
