...
import { Search, Plus, LayoutGrid, List as ListIcon, Trash2, Edit3, Package } from 'lucide-react';
...
const ProductsView = () => {
    ...
    useEffect(() => {
        ...
        if (data.length === 0) {
            ...
            setProducts([
                { nome: 'Hamburguer Gourmet', categoria: 'Main Course', preco: 12.99, custo: 4.50, stock: 50, status: 'Ativo' },
                ...
                { nome: 'Salmao Grelhado', categoria: 'Main Course', preco: 18.00, custo: 7.50, stock: 15, status: 'Baixo Stock' }
            ]);
        } else {
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/5 p-8 rounded-[32px] border border-white/10">
                <div>
                    <h2 className="text-3xl font-black text-white tracking-tighter">Gestao de Produtos</h2>
                    <p className="text-white/40 text-sm font-medium">Controla o teu menu e inventario com precisao.</p>
                </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-white/5 text-[10px] uppercase tracking-widest text-white/40 font-black">
                                    ...
                                    <th className="px-6 py-6 text-right">Preco / Custo</th>
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col gap-1 w-24">
                                                <div className="flex justify-between text-[8px] font-black uppercase tracking-tighter text-white/30">
                                                    <span>Nivel</span>
                                                    <span>{p.stock ?? 0}un</span>
                                                </div>
                                                <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full ${(p.stock ?? 0) < 20 ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'bg-primary shadow-[0_0_8px_rgba(var(--primary-rgb),0.5)]'}`}
                                                        style={{ width: `${Math.min(p.stock ?? 0, 100)}%` }}
                                                    ></div>
                                                </div>
                                            </div>
                                        </td>
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center pt-6 border-t border-white/5">
                                <div>
                                    ...
                                    <p className={`text-sm font-black ${(p.stock ?? 0) < 20 ? 'text-red-500' : 'text-primary'}`}>{p.stock ?? 0} unidades</p>
                                </div>
                                <div className="text-right">
