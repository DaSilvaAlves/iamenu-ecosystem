...
const ReputacaoOnlineView = () => {
  ...
  const [_reviews, setReviews] = useState<ReviewData[]>(MOCK_REVIEWS);

  const handleSelectReview = (review: ReviewData) => {
    setSelectedReview(review);
  };

  const handlePostResponse = (id: string, response: string) => {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'inbox':
        return <Inbox onSelectReview={handleSelectReview as (review: Record<string, unknown>) => void} />;
      case 'settings':
