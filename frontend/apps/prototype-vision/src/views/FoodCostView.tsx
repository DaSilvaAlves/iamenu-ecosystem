...
interface FichaTecnica {
    ...
    categoria: string;
    rendimento: number;
    pesoPorPorcao?: number;
    tempoPreparoTotal?: number;
    tempoCoccao?: number;
    modoPreparo?: string;
    instrucoesEmpratamento?: string;
    loucaRecomendada?: string;
    validade?: string;
    outrosCustosPercentagem?: number;
...
const FoodCostView = () => {
    ...
    const dadosIniciais: FichaTecnica[] = [
        {
            id: 'ft-001',
            nomePrato: 'Bacalhau a Bras',
            categoria: 'Peixe',
            rendimento: 4,
            pesoPorPorcao: 350,
            ...
            tags: 'Classico, Signature'
        },
        {
            id: 'ft-002',
            nomePrato: 'Bacalhau a Lagareiro',
            categoria: 'Peixe',
            rendimento: 1,
            pesoPorPorcao: 400,
    ...
    // 2. LOGICA DE CALCULO (O MOTOR)
    const calcularMetricas = (ficha: FichaTecnica): Metricas => {
        if (!ficha || !ficha.ingredientes) return { custoIngredientes: 0, custoPorPorcao: 0, precoSugerido: 0, margemReal: 0, lucroPorPrato: 0, isLowMargin: true };
    ...
    // 3. HANDLERS CRUD
    const handleSaveFicha = (novaFicha: FichaTecnica) => {
        if (novaFicha.id) {
    ...
    const handleDelete = (id: string) => {
        if (window.confirm("Tem certeza que deseja eliminar esta ficha tecnica?")) {
    ...
    const handleExportPDF = (ficha: FichaTecnica) => {
        const doc = new jsPDF();
        ...
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.text('METRICAS FINANCEIRAS', 15, yPos);
        yPos += 8;

        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.text(`Custo por Porcao: \u20AC${metricas.custoPorPorcao.toFixed(2)}`, 20, yPos);
        yPos += 6;
        doc.text(`Preco de Venda: \u20AC${ficha.precoVendaAtual.toFixed(2)}`, 20, yPos);
        yPos += 6;
        ...
        yPos += 6;
        doc.text(`Preco Sugerido: \u20AC${metricas.precoSugerido.toFixed(2)}`, 20, yPos);
        yPos += 12;
        ...
        if (ficha.modoPreparo && yPos < 250) {
            ...
            doc.setFontSize(10);
            doc.setFont('helvetica', 'normal');
            const lines: string[] = doc.splitTextToSize(ficha.modoPreparo, 180);
        ...
        if (yPos < 250) {
            doc.setFontSize(14);
            doc.setFont('helvetica', 'bold');
            doc.text('INFORMACOES OPERACIONAIS', 15, yPos);
            yPos += 8;
            doc.setFontSize(10);
            doc.setFont('helvetica', 'normal');
            doc.text(`Tempo Preparo: ${ficha.tempoPreparoTotal || 0} min`, 20, yPos);
            yPos += 6;
            doc.text(`Tempo Coccao: ${ficha.tempoCoccao || 0} min`, 20, yPos);
            yPos += 6;
            doc.text(`Validade: ${ficha.validade || 'N/A'}`, 20, yPos);
            yPos += 6;
            doc.text(`Louca: ${ficha.loucaRecomendada || 'Padrao'}`, 20, yPos);
        }
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            doc.text(`Ficha Tecnica \u2022 iaMenu Food Cost System \u2022 Pagina ${i}/${pageCount}`, 15, 290);
        }
        ...
        <div className="p-8 space-y-8 max-w-7xl mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-8 glass-effect rounded-[32px] border border-white/10 shadow-2xl relative overflow-hidden">
                ...
                <div>
                    ...
                    <p className="text-white/40 font-bold text-sm tracking-wide">Gestao Inteligente de Margens e Engenharia de Menu.</p>
                </div>
                ...
                >
                    <PlusCircle size={20} /> Nova Ficha Tecnica
                </button>
            </div>
            ...
            <div className="flex p-1 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 w-fit">
                ...
                >
                    <BarChart3 size={16} /> Analise de Menu
                </button>
                <button
                                            ...
                                            <div key={f.id} className="group p-5 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-primary/30 transition-all flex items-center justify-between">
                                                <div className="flex items-center gap-5">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] text-white/20 uppercase font-black tracking-widest">{f.categoria} &bull; {f.rendimento} doses</p>
                                                    </div>
                                                </div>
                                                <div className="text-right">
                                                    ...
                                                    <p className={`text-[10px] font-black uppercase tracking-tighter ${m.isLowMargin ? 'text-red-400' : 'text-primary'}`}>
                                                        {m.isLowMargin ? 'Ladrao de Lucro' : 'Margem Excelente'}
                                                    </p>
                                                </div>
                                            </div>
                                        );
                            ...
                            <div className="p-8 glass-effect rounded-[40px] border border-white/5 bg-primary/5">
                                ...
                                <div className="space-y-6">
                                    <div className="p-6 rounded-3xl bg-red-500/5 border border-red-500/10 group cursor-help hover:bg-red-500/10 transition-all">
                                        ...
                                        <p className="text-[11px] text-white/60 leading-relaxed">O custo do <strong>Bacalhau Especial</strong> subiu 15% nos ultimos 30 dias. A margem do prato esta agora em niveis criticos (58%). Recomendamos reajuste imediato.</p>
                                    </div>
                                    ...
                                    <div className="p-6 rounded-3xl bg-green-500/5 border border-green-500/10 hover:bg-green-500/10 transition-all">
                                        ...
                                        <p className="text-[11px] text-white/60 leading-relaxed">Prato <strong>Bacalhau a Bras</strong> tem alta demanda e gordura na margem. Criar uma oferta &quot;Combo Familiar&quot; pode aumentar o volume em 22% sem degradar o lucro.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-1 glass-panel p-4 rounded-[32px] border border-white/5 space-y-4 h-fit">
                            ...
                            <div className="px-2 space-y-2">
                                <select
                                    ...
                                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFilterCategoria(e.target.value)}
                                ...
                                <select
                                    value={filterMargem}
                                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFilterMargem(e.target.value)}
                                    className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white text-xs font-bold outline-none focus:border-primary/50 transition-all"
                                >
                                    <option value="all">Todas Margens</option>
                                    <option value="alta">Alta Margem (&ge;65%)</option>
                            ...
                            {selectedFicha ? (
                                <div className="glass-panel p-10 rounded-[40px] border border-white/5 bg-white/[0.01] space-y-12">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                        <MetricaCard label="Custo Producao" value={`${metricasSelect.custoPorPorcao.toFixed(2)}\u20AC`} icon={Utensils} />
                                        <MetricaCard label="Preco Praticado" value={`${selectedFicha.precoVendaAtual.toFixed(2)}\u20AC`} icon={DollarSign} />
                                        <MetricaCard
                                            label="Margem Real"
                                                ...
                                                <table className="w-full text-left text-xs">
                                                    <thead>
                                                        <tr className="bg-white/5 text-white/30 font-black uppercase tracking-widest border-b border-white/5">
                                                            ...
                                                            <th className="p-4 text-right">Custo Liq.</th>
                                                        </tr>
                                                    </thead>
                                            ...
                                            <div className="glass-panel p-8 rounded-[32px] border border-white/5 space-y-6">
                                                ...
                                                <div className="grid grid-cols-2 gap-6">
                                                    <div className="space-y-1">
                                                        <p className="text-[9px] text-white/20 uppercase font-black">Tempo Medio Prepara</p>
                                                        <p className="text-lg font-black text-white italic">{selectedFicha.tempoPreparoTotal || 0} min</p>
                                                    </div>
                                                    <div className="space-y-1">
                                                        <p className="text-[9px] text-white/20 uppercase font-black">Tempo de Fogo</p>
                                                        <p className="text-lg font-black text-white italic">{selectedFicha.tempoCoccao || 0} min</p>
                                                    </div>
                                                    <div className="space-y-1">
                                                        <p className="text-[9px] text-white/20 uppercase font-black">Validade</p>
                                                        <p className="text-lg font-black text-white italic">{selectedFicha.validade || 'N/A'}</p>
                                                    </div>
                                                    <div className="space-y-1">
                                                        <p className="text-[9px] text-white/20 uppercase font-black">Louca</p>
                                                        <p className="text-[11px] font-bold text-white/60">{selectedFicha.loucaRecomendada || 'Padrao'}</p>
                                                    </div>
                                                </div>
                                            </div>

                                            <div className="p-8 rounded-[32px] bg-white/[0.01] border border-white/5 space-y-4">
                                                <h4 className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em]">Marketing & Alergenios</h4>
                                                ...
                                                <div className="glass-panel p-8 rounded-[32px] border border-white/5 space-y-4">
                                                    ...
                                                    {selectedFicha.loucaRecomendada && (
                                                        <div className="pt-4 border-t border-white/5">
                                                            <p className="text-[9px] text-white/30 uppercase font-black mb-1">Louca Recomendada:</p>
                                ...
                                <div className="p-24 glass-panel rounded-[40px] border border-white/5 text-center space-y-6">
                                    ...
                                    <p className="text-white/40 font-bold uppercase tracking-widest text-xs">Cria ou seleciona uma ficha tecnica para comecar.</p>
                                </div>
                            )}
                        </div>
                    </motion.div>
                            ...
                            <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                                <div>
                                    <h2 className="text-2xl font-black text-white uppercase italic tracking-tighter">
                                        {selectedId ? 'Editar Ficha Tecnica' : 'Construir Nova Receita'}
                                    </h2>
                                    <p className="text-white/20 text-[10px] font-black uppercase tracking-widest">Motor de Calculo iaMenu Profit</p>
                                </div>
...
const MetricaCard = ({ label, value, icon: Icon, highlight = "text-white" }: MetricaCardProps) => (
...
const EditorForm = ({ initialData, onSave, onCancel }: EditorFormProps) => {
    const [form, setForm] = useState<FichaTecnica>({
        id: '',
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-12 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <InputGroup label="Nome do Prato" placeholder="Ex: Arroz de Polvo Malandro">
                    <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, nomePrato: e.target.value })}
                ...
                <InputGroup label="Categoria">
                    <select
                        ...
                        onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setForm({ ...form, categoria: e.target.value })}
                        className="editor-input"
                    >
                        <option value="Entrada">Entrada</option>
                        <option value="Peixe">Peixe</option>
                ...
                <InputGroup label="Doses (Rendimento)">
                    <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, rendimento: parseInt(e.target.value) })}
                        className="editor-input"
                    />
                </InputGroup>
            </div>
                        ...
                        >
                            {form.foto ? (
                                ...
                                <>
                                    ...
                                    <p className="text-white/20 text-xs mt-2">JPG, PNG, WEBP ate 5MB</p>
                                </>
                            )}
                        </label>
                    </div>
                ...
                <div className="space-y-3">
                    {form.ingredientes.map((ing, idx) => (
                        <div key={ing.id} className="grid grid-cols-1 md:grid-cols-6 gap-4 p-4 bg-white/5 rounded-2xl items-end relative group">
                            ...
                            <div>
                                <label className="text-[9px] font-black text-white/20 uppercase mb-2 block">Preco Ref (\u20AC)</label>
                                <input
                                    type="number"
                                    step="0.01"
                                    className="editor-input text-xs"
            ...
            {/* Bloco 3: Financeiro e Estrategico */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <InputGroup label="Margem Bruta Desejada (%)">
                        <input
                            ...
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, margemLucroDesejada: parseInt(e.target.value) })}
                        />
                    </InputGroup>
                    <InputGroup label="Overhead / Custos Fixos (%)">
                        <input
                            ...
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, outrosCustosPercentagem: parseInt(e.target.value) })}
                        />
                    </InputGroup>
                    <InputGroup label="Preco de Venda Real (\u20AC)">
                        <input
                            type="number"
                            step="0.05"
                            className="editor-input text-xl text-white font-black"
                            value={form.precoVendaAtual}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, precoVendaAtual: parseFloat(e.target.value) })}
                        />
                    </InputGroup>
                </div>
            </div>
            ...
            <div className="flex gap-4 pt-10 border-t border-white/5">
                ...
                <button type="submit" className="flex-[2] py-5 bg-primary text-black font-black uppercase italic tracking-tighter flex items-center justify-center gap-3 rounded-[24px] shadow-2xl shadow-primary/20 hover:scale-[1.02] transition-all">
                    <Save size={20} /> Guardar Configuracao Inteligente
                </button>
            </div>
        </form>
    );
