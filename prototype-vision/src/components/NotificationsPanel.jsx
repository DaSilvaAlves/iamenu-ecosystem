...
const NotificationsPanel = ({ isOpen, onClose }) => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/v1/community/notifications?limit=20', {
        headers: {
  ...
  const markAsRead = async (id) => {
    try {
      const response = await fetch(`http://localhost:3001/api/v1/community/notifications/${id}/read`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const markAllAsRead = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/v1/community/notifications/read-all', {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const deleteNotification = async (id) => {
    try {
      const response = await fetch(`http://localhost:3001/api/v1/community/notifications/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
