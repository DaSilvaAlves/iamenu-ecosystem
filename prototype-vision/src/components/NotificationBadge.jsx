...
const NotificationBadge = ({ onClick, className = '' }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`relative flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 transition-colors ${className}`}
      aria-label={`Notifications ${unreadCount > 0 ? `(${unreadCount} unread)` : ''}`}
      style={{ overflow: 'visible' }}
    >
      <Bell size={18} className="text-white/60" />
      <span className="text-sm text-white/60">Notificações</span>
